...
class import_export_excel_to_oracle_class(FlaskForm):
    """
    ...
    b_copy_from_excel_to_oracle = SubmitField('Copy Excel file into Oracle DB')
...
class excel_to_oracle_class():
    ...
    def __init__(self,
        ...
        if p_table_name == '':
            self.message_error = 'Oracle table name is required!'
            return None
        ...
        try:
            ...
            self.message_error = 'Format file invalid. Only xlsx file format!'
            #esco
            return None        
        #Se il file contiene più fogli, avverto che verrà preso solo il primo
        if len( wb.sheets() ) > 1:            
            self.message_info = 'This file contains more than one sheet. It will be taken the first ' + wb.sheet_by_index(0).name + '!'
        ...
        if self.creo_tabella(v_definizione_colonne, p_table_name) == 'ok':
            if self.importa_foglio(v_foglio, v_definizione_colonne, p_table_name) == 'ok':
                #Messaggio finale
                self.message_info = 'Action completed with ' + str(self.v_numero_totale_righe) + ' records imported!'
    ...
    def creo_tabella(self, 
        ...
        try:    
            ...
            self.message_error = "Problem during create Oracle table!" + chr(10) + "The table " + p_table_name + " already exists?" + chr(10) + "Remember that the excel file must be haven't formatting and filters activated!"
            #esco
            return 'ko'
...
if __name__ == "__main__":    
    a =excel_to_oracle_class(False,
                             "SMILE",
                             "SMILE",
                             "BACKUP_815",
                             "CANCELLAMI_165",                                         
                             "N:\\smi_job\\It&c\\mvalaguz\\01 - Aggiornamenti\\2021\\2021 05 12 FBONZI aggiornamento ore CP_DETOR per MISE\\ORE TEST DA RICODIFICARE.xlsx")       
